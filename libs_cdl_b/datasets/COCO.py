...
class COCO(_BaseDataset):
    ...
    def _set_files(self):
        ...
        # self.label_dir = osp.join(self.root, "sem_seg_coco_RIB")
        self.label_dir = osp.join(self.root, "coco_seg_anno")

        if self.split in ["train_2014", "val_2014", "test"]:
            file_list = osp.join(
                self.root, "List", self.split + ".txt"
            )
