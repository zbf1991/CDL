...
class MSC(nn.Module):
    ...
    def forward(self, x):
        # Original
        logits, logits_2nd, features_3rd = self.base(x)
        ...
        for p in self.scales:
            ...
            logits_pyramid_single, logits_pyramid_single_2nd, features_pyramid_single_3rd = self.base(h)
        ...
        if self.training:
            return [logits] + logits_pyramid + [logits_max], [logits_2nd] + logits_pyramid_2nd + [logits_max_2nd],\
                   [features_3rd_dis] + features_pyramid_3rd_dis + [features_mean_3rd_dis]
        else:

            return logits_max/2.0
