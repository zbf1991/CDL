...
from libs_cdl_b.datasets import get_dataset
from libs_cdl_b.models import DeepLabV2_ResNet101_MSC
from libs_cdl_b.utils import DenseCRF, PolynomialLR, scores
import cv2
import libs_cdl_b.utils.tool as tool
from libs_cdl_b.utils.dice_loss import DiceLoss
...
def train(config_path, cuda):
    ...
    for iteration in tqdm(
        ...
        for ITER_CURRENT in range(CONFIG.SOLVER.ITER_SIZE):
            ...
            # Propagate forward
            logits, logits_2nd, fts_distance = model(images.cuda())
            ...
            for num, logit in enumerate(logits):
                ...
                fts_distance_signle = fts_distance[num]
                ...
                main_label_3rd = labels_.clone()

                if iteration > CONFIG.THRESHOLDS.START_RATION*CONFIG.SOLVER.ITER_MAX:
                    main_label, main_label_2nd, main_label_3rd = tool.UpDate_labels(logit, logit_2nd, main_th, aux_th, main_label, fts_distance_signle)
                    aff_single_loss = tool.compute_affinity_loss(fts_distance_signle, main_label_3rd)
                    iter_loss_3rd_single = aff_single_loss
                    iter_loss_3rd += CONFIG.THRESHOLDS.AFF_LOSS_WEIGHT*iter_loss_3rd_single
                else:
                    aff_single_loss = tool.compute_affinity_loss(fts_distance_signle, main_label_3rd)
                    iter_loss_3rd_single = aff_single_loss
                    iter_loss_3rd += CONFIG.THRESHOLDS.AFF_LOSS_WEIGHT*iter_loss_3rd_single
                ...
                if CONFIG.THRESHOLDS.IMAGENET_INIT == True:
                    iter_loss_single = criterion_balance(logit, main_label)
                    ...
                    if iteration < CONFIG.THRESHOLDS.START_RATION * CONFIG.SOLVER.ITER_MAX:
                        ...
                        iter_loss_2nd_single = criterion_2nd(logit_2nd, main_label_2nd)
                        iter_loss_2nd += CONFIG.THRESHOLDS.LOSS_WEIGHT_2ND * iter_loss_2nd_single
                else:
                    iter_loss_single = criterion(logit, main_label)
                    iter_loss_1st += CONFIG.THRESHOLDS.LOSS_WEIGHT_1ST*iter_loss_single
            ...
            iter_loss = (iter_loss_1st + iter_loss_2nd + iter_loss_3rd) / CONFIG.SOLVER.ITER_SIZE

            if (CONFIG.THRESHOLDS.IMAGENET_INIT) == True and (iteration < 100):
                iter_loss = iter_loss * (iteration / 100 + 0.01)

            # print('iter_loss: ', iter_loss_1st.item(), 'iter_2nd_loss: ', iter_loss_2nd.item(), 'iter_3rd_loss: ', iter_loss_3rd.item())
...
def test(config_path, model_path, cuda):
    ...
    for image_ids, images, gt_labels in tqdm(
        ...
        # label_save_path = '/home/fmp/DataDisk/RRM/val_nocrf'
